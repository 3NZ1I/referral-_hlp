...
const outputPath = path.resolve(__dirname, '..', 'src', 'data', 'selectOptions.generated.json');
...
fs.writeFileSync(outputPath, JSON.stringify(selectOptions, null, 2), 'utf8');
console.log(`Wrote ${Object.keys(selectOptions).length} select lists to ${outputPath}`);
