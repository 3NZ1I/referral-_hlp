...
const remapRosterHeader = (rawCell = '') => {
  ...
  // try to match "partnernu1_7_1 name" or "7_1 name" and many variants
  ...
  // try with label then slot pattern: "First name 7.1" or "First name 7-1"
  ...
  // try slot at end: "First name (7.1)"
  ...
  // Last resort: detect just slot number with a label fragment
  const token4 = stripped.match(/(\d+[._-]?\d*)/);
  if (token4) {
    const rawSlot = token4[1].replace(/[._-]/g, '_').replace(/^0+/, '');
    // find label by removing slot
...
console.log('Index | Original header | normalized | canonical remap | roster canonical (if any)');
...
mappedHeaders.forEach((h, idx) => {
  console.log(`${idx.toString().padStart(3, ' ')} | ${h.original} | ${h.normalized} | ${h.aliasCanonical || '-'} | ${h.rosterCanonical || '-'} `);
});

console.log('\nFirst data row sample:');
const sample = rows[2] || rows[1] || [];
console.log(JSON.stringify(sample.slice(0, 40), null, 2));

console.log('\nPotential roster columns found:');
mappedHeaders.forEach((h, idx) => {
  if (h.rosterCanonical) console.log(`${idx} | ${h.original} -> ${h.rosterCanonical}`);
});

console.log('\nAny header normalized to an alias canonical?');
mappedHeaders.forEach((h, idx) => {
  if (h.aliasCanonical) console.log(`${idx} | ${h.original} -> ${h.aliasCanonical}`);
});
